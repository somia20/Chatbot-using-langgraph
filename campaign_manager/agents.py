...
def is_valid_date(date_str):
    ...
    if date_str == "" or date_str == "null" or date_str == "none":
        return False
    return True
...
def campaign_manager(state: OverallState):
    ...
    if current_stage == "rule_service":
        ...
        if segment_step.status == "completed" and action_step.status == "completed":
            ...
            try:
                ...
                response = requests.post(rule_service_url, json=request_body, timeout=45)
    ...
    if current_stage == "schedule":
        ...
        if is_complete:
            ...
            state[
                "messages"] = "Great! Your campaign schedule has been set up. What would you like to name this campaign?"
            print("Schedule information complete. Moving to campaign name stage.")
        else:
            schedule_info.status = "in_progress"
            # Ask for missing information - more concise version
            missing_info_text = ", ".join(missing_info)
            state["messages"] = f"I still need {missing_info_text}. Could you provide that?"
            return state

    # STAGE 3: Collect and Validate Campaign Name
    if current_stage == "campaign_name":
        api_call = False
        dates_shedule = state["campaign_manager_state"].campaign_dates

        if dates_shedule:
            user_input = state.get("user_input", "")
            structured_llm_dates = llm.with_structured_output(CampaignDateExtraction)
            prompt = f"""
                       Extract a campaign dates
                       "{user_input}"
                       ...
                       Return the extracted campaign dated and a confidence score between 0.0 and 1.0.
                       Use 0.0 for no relevant information found, 
                       if no information is 
                       """

            # Extract campaign name
            extracted_dates = structured_llm_dates.invoke([SystemMessage(content=prompt)])
            if extracted_dates.confidence > 0.6:
                # Update the schedule info in the state
                schedule_info = state["campaign_manager_state"].schedule_info

                if not is_valid_date(schedule_info.start_date) and extracted_dates.start_date:
                    schedule_info.start_date = extracted_dates.start_date

                # Only update end_date if the current one isn't valid but the extracted one is
                if not is_valid_date(schedule_info.end_date) and extracted_dates.end_date:
                    schedule_info.end_date = extracted_dates.end_date

                # Check if we now have all the required date information
                if (is_valid_date(schedule_info.start_date) and is_valid_date(schedule_info.end_date)):
                    # We have all dates and they are valid, we can proceed to finalize
                    dates_shedule = False

                    # Generate summary with the campaign name and dates
                    candidate_name = state["campaign_manager_state"].campaign_name
                    summary = create_campaign_summary(candidate_name, state)

                    # Move to finalize stage
                    state["campaign_manager_state"].stage = "finalize"
                    # state["messages"] = f"""Great! Your campaign "{candidate_name}" is scheduled from {schedule_info.start_date} to {schedule_info.end_date}.
                    #
                    #         {summary}
                    #
                    ...
                    #         """
                    state[
                        "messages"] = f"""Great! Your campaign "{candidate_name}" is scheduled from {schedule_info.start_date} to {schedule_info.end_date}.Please confirm if these details are correct.
                                    """
                    return state
                else:
                    # Still missing some date information, ask again
                    missing_dates = []
                    if not is_valid_date(schedule_info.start_date):
                        missing_dates.append("start date")
                    if not is_valid_date(schedule_info.end_date):
                        missing_dates.append("end date")
                    mess = f"I need the campaign  {' and '.join(missing_dates)} to proceed. "
                    state[
                        "messages"] = f"I need the campaign {' and '.join(missing_dates)} to proceed. "
                    return state
            else:
                # Couldn't extract dates with confidence, ask explicitly
                state["messages"] = "I need the campaign start and end dates."
                return state
            ...
            if extracted_name.confidence > 0.5 and extracted_name.campaign_name:
                ...
                try:
                    ...
                    if validation_result["operationCode"] == 0:
                        ...
                        if not state["campaign_manager_state"].schedule_info.start_date:
                            state["messages"] = " when you want to launch? provide start date and end date "
                            state["campaign_manager_state"].campaign_dates = True
                            return state
                        ...
                        state["campaign_manager_state"].stage = "finalize"
                        state["messages"] = f"""
                        Great! I'll name this campaign "{candidate_name}".{summary} Please confirm if these details are correct or let me know what you'd like to change."""
                ...
                state["messages"] = "What would you like to name this campaign?"
                return state
    ...
    if current_stage == "finalize":
        try:
            ...
            if response_data.get("operationCode") == 0:
                ...
                campaign_info.stage = "completed"
                state["campaign_manager_state"].launch_campaign = "true"
...
def create_campaign_summary(campaign_name: str, state: OverallState) -> str:
    """Create a clean, user-friendly summary of the campaign details."""
    ...
    if schedule_info.schedule_type == "daily":
        schedule_details = f"daily at {schedule_info.time or 'unspecified time'}"
    elif schedule_info.schedule_type == "weekly":
        days = ", ".join(schedule_info.days_of_week) if schedule_info.days_of_week else "unspecified days"
        schedule_details = f"weekly on {days}"
    elif schedule_info.schedule_type == "monthly":
        days = ", ".join(
            [str(d) for d in schedule_info.days_of_month]) if schedule_info.days_of_month else "unspecified days"
        schedule_details = f"monthly on {days}"

    # Create a structured prompt with all campaign details
    # prompt = f"""
    # Generate a single paragraph summary of this marketing campaign.
    # Keep it brief, conversational and easy to read. Only include facts provided here - no extra information or suggestions.
    #
    # Campaign details:
    # - Name: {campaign_name}
    # - Target Segment: {segment_step.collected_info.get('segment_condition', 'Not specified')}
    # - Reward Type: {action_step.collected_info.get('action_type', 'Not specified')}
    # - Reward Value: {action_step.collected_info.get('reward_value', 'Not specified')}
    # - Reward Duration: {action_step.collected_info.get('duration_days', 'Not specified')} days
    # - Communication Channels: {channel_step.collected_info.get('channels', 'Not specified')}
    # - Message Frequency: {channel_step.collected_info.get('frequency', 'Not specified')}
    # - Schedule Type: {schedule_info.schedule_type or 'Not specified'}
    # - Schedule Details: {schedule_details}
    # - Start Date: {schedule_info.start_date or 'Not specified'}
    # - End Date: {schedule_info.end_date or 'Not specified'}
    #
    # Important: Return exactly ONE single sentence with NO line breaks, NO extra spaces, and NO HTML or markdown formatting.
    # """
    prompt = f"""
    Generate a single paragraph summary of this marketing campaign.
    Keep it brief, conversational and easy to read. Only include facts provided here - no extra information or suggestions.
    ...
    Important: Return exactly ONE single sentence with NO line breaks, NO extra spaces, and NO HTML or markdown formatting. no /n 
    """
    ...
    # Remove any HTML, markdown, or extra artifacts
    summary = summary.replace("</div>", "").replace("<div>", "")

    # Additional cleaning to ensure one continuous line
...
def follow_up(state: OverallState):
    ...
    if campaign_info.stage == "waiting" and pending_steps:
        ...
        # Create a prompt for the LLM that encourages brief, conversational responses
        prompt = f"""
        I'm helping a user create a marketing campaign and need to ask for missing information.

        Based on what they've told me so far, I need to ask about:
        {chr(10).join(questions)}

        Create a friendly, BRIEF message asking for this information. Keep it under 3 sentences, conversational, 
        and to the point. Don't use bullet points. Don't introduce yourself or be overly formal.
        Focus only on the immediate next piece of information needed.
        Note : make it short as possible 
        no need for addressing and all , no need for examples  
        """
...
def extract_actions(state: ExtractState):
    ...
    - it can be  a product like data product voice product etc
...
def create_campaign_confirmation_summary(campaign_name: str, state: OverallState) -> str:
    ...
    summary.extend([
        f"**Start Date**: {schedule_info.start_date or 'Not specified'}",
        f"**End Date**: {schedule_info.end_date or 'Not specified'}",
        f"",
